...
> **Note**: Ensure the AWS CLI user (`aws configure`) or Terraform assumed role has sufficient permissions to manage **S3**, **Lambda**, and **IAM resources**.  
...
The Lambda function and notebook commands are taken directly from the author's original implementation (environment variables added to Lambda function for portability).  
